...
const CFG = {
  ...
  rulesUrl: 'https://desk.famerp.ru/rules/'
};
...
async function scrapeRules(context) {
  const page = await context.newPage();
  // 1) Идем на главную desk и пытаемся найти реальную ссылку на правила
  await page.goto(CFG.deskHome || CFG.rulesUrl, { waitUntil: 'networkidle', timeout: 60000 }).catch(() => null);
  await page.waitForTimeout(900);
  let rulesUrl = await discoverRulesUrl(page);
  const base = new URL(page.url() || (CFG.deskHome || CFG.rulesUrl));
  const fallbacks = [
    CFG.rulesUrl,
    new URL('/rules/', base).toString(),
    new URL('/rules', base).toString(),
    new URL('/rulebook/', base).toString(),
    new URL('/documents/rules', base).toString()
  ].filter(Boolean);
  if (!rulesUrl) rulesUrl = fallbacks[0];
  if (rulesUrl && rulesUrl !== page.url()) {
    await page.goto(rulesUrl, { waitUntil: 'networkidle', timeout: 60000 }).catch(() => null);
    await page.waitForTimeout(1200);
  }

  // 2) Берем максимально "чистый" текст: main/article/content, иначе весь body с меню/футером
