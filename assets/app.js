(function(){
  ...
  const runStaff=()=>{
    ...
    const card=(p)=>{
      ...
      const time=safeText(p.playtime);
      const last=safeText(p.last_seen);
      ...
      const badges=[
        ...
        time!=='-'?'<span class="badge">'+time.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</span>':'',
  ...
  const runBans=()=>{
    ...
    const cards=byId('bansCards');
    if(!tbody||!cards) return;
    const search=byId('bansSearch');
    const filters=byId('bansFilters');
    const refresh=byId('refreshBans');
    let data=[];
    let filter='active';

    const statusLabel=(b)=>{
      const st=safeText(b.status);
      if(st!=='-') return st;
      if(b.expires_at){
        const d=parseTime(b.expires_at);
        if(d && d.getTime()<Date.now()) return 'Истек';
        return 'Активен';
      }
      return b.active===false?'Истек':'Активен';
    };
    ...
    const row=(b)=>{
      ...
      const dt=safeText(b.date);
      const len=safeText(b.length);
      const st=statusLabel(b);
      return '<tr>'+
        '<td>'+who.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</td>'+
        '<td>'+why.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</td>'+
        '<td>'+by.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</td>'+
        '<td>'+dt.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</td>'+
        '<td>'+len.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</td>'+
        '<td>'+st.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</td>'+
      '</tr>';
    };
    ...
    const card=(b)=>{
      ...
      const len=safeText(b.length);
      const st=statusLabel(b);
      const badge=st==='Активен'?'<span class="badge bad">Активен</span>':'<span class="badge">Истек</span>';
      return '<div class="card padded">'+
        '<div style="display:flex; justify-content:space-between; gap:10px; align-items:flex-start">'+
          '<div style="min-width:0">'+
            '<div style="font-weight:850; white-space:nowrap; overflow:hidden; text-overflow:ellipsis">'+who.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</div>'+
            '<div class="muted" style="margin-top:4px">'+why.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</div>'+
          '</div>'+
          '<div>'+badge+'</div>'+
        '</div>'+
        '<div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:10px">'+
          (by!=='-'?'<span class="badge">'+by.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</span>':'')+
          (dt!=='-'?'<span class="badge">'+dt.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</span>':'')+
          (len!=='-'?'<span class="badge">'+len.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</span>':'')+
        '</div>'+
      '</div>';
    };
    ...
    const apply=()=>{
      const q=(search?search.value:'').trim().toLowerCase();
      let rows=data;
      if(filter==='active') rows=rows.filter(x=>statusLabel(x)==='Активен');
      if(filter==='expired') rows=rows.filter(x=>statusLabel(x)!=='Активен');
      if(q) rows=rows.filter(x=>{
        const s=(String(x.player||'')+' '+String(x.reason||'')+' '+String(x.admin||'')+' '+String(x.steamid||'')).toLowerCase();
        return s.includes(q);
      });
      if(rows.length===0){
        tbody.innerHTML='<tr><td colspan="6" class="muted">Нет данных</td></tr>';
        cards.innerHTML='<div class="muted" style="padding:12px">Нет данных</div>';
        return;
      }
      tbody.innerHTML=rows.map(row).join('');
      cards.innerHTML=rows.slice(0,120).map(card).join('');
    };

    const load=async()=>{
      try{
        const d=await fetchJson(apiBase+'bans.php');
        if(!d||!d.ok) return;
        data=Array.isArray(d.bans)?d.bans:[];
        mountLive('bansLive','bansUpdated',d.updated_at||d.last_update);
        apply();
      }catch(_){ }
    };
    ...
    if(filters){
      qsa('.pill',filters).forEach(p=>{
        p.addEventListener('click',()=>{
          qsa('.pill',filters).forEach(x=>x.classList.remove('active'));
          p.classList.add('active');
          filter=p.getAttribute('data-filter')||'active';
          apply();
        });
      });
    }
